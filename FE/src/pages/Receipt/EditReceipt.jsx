...
function EditReceipt() {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        ...
        const supplierOptions = supplierResponse.data;
  ...
  const handleSuccessReceipt = async () => {
    setIsSubmit(true);
    try {
      await axios.put(`${config.API_HOST}/material-receipts/update/${id}`, {
      ...
      toast.success("Cập nhật biên lai nhập nguyên liệu thành công!");
      navigate(config.routes.receiptList);
    } catch (error) {
      toast.error(
        error?.response?.data?.message || "Cập nhật biên lai không thành công!"
      );
    } finally {
      setIsSubmit(false);
    }
  ...
  if (receipt == null) return <>Lấy dữ liệu biên lai không thành công!</>;
  return (
    <>
              ...
              <Table.Body className="divide-y">
                {rows.map((row) => (
                  ...
                  >
                    <Table.Cell className="whitespace-nowrap text-white font-medium dark:text-white w-[40%]">
                      <Select
                        ...
                        defaultValue={materialOptions.find(
                          (option) => option.value === row?.material?.value
                        )}
                        onChange={(selectedOption) =>
                          handleSelectChange(selectedOption, row.id)
                        }
                        className="py-1 text-[rgb(235,238,244)]"
                        theme={customTheme}
                        styles={customStyles}
                      />
                    </Table.Cell>
                    ...
                    <Table.Cell className="bg-blue text-white">
                      <span
                        className="flex items-center justify-center border border-[rgb(235,238,244)] w-6 h-6 cursor-pointer"
        ...
        <div className="rounded-lg  col-span-2 border border-[rgb(60,78,113)] bg-[rgb(42,55,79)]">
          ...
          <div className="p-4">
            <div className="mb-4 block">
              ...
              <Select
                ...
                defaultValue={supplierOptions.find(
                  (option) => option.value === receipt.supplier.id
                )}
            ...
            <label className="inline-flex items-center cursor-pointer">
              ...
              <span className="ms-3 text-sm text-white font-medium dark:text-gray-300">
                Đã thanh toán
              </span>
            </label>
            <button
